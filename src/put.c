...
size_t	ft_put_str(char *s)
{
	...
	if (s == NULL)
		printed_char_count += ft_put_str("(null)");
...
size_t	ft_put_nbr(int n)
{
	...
	printed_char_count = 0;
	if (n == INT_MIN)
		printed_char_count += ft_put_str("-2147483648");
	else if (n < 0)
	{
		printed_char_count += ft_put_char('-');
...
size_t	ft_put_hex(unsigned long n, bool put_in_capital)
{
	...
	printed_char_count = ft_put_str(converted_num);
	return (printed_char_count);
}
...
size_t	ft_put_address(unsigned long address)
{
	...
	printed_char_count = 0;
	printed_char_count += ft_put_str(HEX_PREFIX);
